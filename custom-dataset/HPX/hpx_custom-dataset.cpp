...
struct Options {
  ...
  std::string datasetPath = "/Volumes/ANDROID/Pytorch/custom-dataset/dataset/";
...
std::mt19937 urng(rng());
  ...
  std::shuffle(train.begin(), train.end(), urng);

  std::random_device mrng;
std::mt19937 krng(mrng());
  std::shuffle(test.begin(), test.end(), krng);
...
void train(
  ...
  for (auto& batch : loader) {
    ...
    if (index++ % options.log_interval == 0) {
      ...
      hpx::cout << "Train Epoch: " << epoch << " " << end << "/" << data_size
...
void test(Network& network, DataLoader& loader, size_t data_size) {
  ...
  if (index++ % options.log_interval == 0)
    hpx::cout << "Test Loss: " << Loss / data_size
              << "\tAcc: " << Acc / data_size << hpx::endl;
}
...
int main() {
  ...
  hpx::cout << "Running on: "
            << (options.device == torch::kCUDA ? "CUDA" : "CPU") << hpx::endl;

  /*hpx::future<std::pair<Data, Data>> fnew = hpx::async(readInfo);
  auto data = fnew.get();*/
  auto data = readInfo();
  auto train_set =
